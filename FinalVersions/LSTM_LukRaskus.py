...
data = getDataFromFile(r'D:\!Ilya\MacineLearning\OxaProject\PJME_hourly.csv')
...
lookback = 20  # на сколько шагов назад смотрим при предсказании
...
# Модель 2
model = Sequential()
model.add(LSTM(4, activation='linear', input_shape=(lookback, feature)))
# model.add(LSTM(8))
...
for i in range(count_of_predict):
    plt.plot(yhat[0:300, i], label='Npredict' + str(i + 1))
plt.plot(yTest[0:300, 0], label='Ntrue')
plt.legend()
plt.show()
# Преобразовываем данные в исходный формат для проверки RMSE


plt.plot(yhat[0:20, 0], label='Npredict' + str(1))
plt.plot(yhat[0:20, 1], label='Npredict' + str(2))
plt.legend()
plt.show()

for i in range(count_of_predict):
    plt.plot(yhat[0:300, i], label='Npredict' + str(i + 1))
    plt.plot(yTest[0:300, i], label='Ntrue')
    plt.legend()
    plt.show()
