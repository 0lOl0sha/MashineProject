...
data = getDataFromFile(r'D:\!Ilya\MacineLearning\OxaProject\DUQ_hourly.csv')
...
lookback = 3  # на сколько шагов назад смотрим при предсказании
feature = 1  # входной вектор
count_of_predict = 2  # сколько предсказываем
...
# Модель 2
model = Sequential()
model.add(LSTM(100, activation='relu', input_shape=(lookback, feature), return_sequences=True))
model.add(LSTM(100, activation='relu'))
model.add(Dense(count_of_predict))
model.compile(optimizer='adam', loss='mse')
history = model.fit(xTrain, yTrain, batch_size=2, epochs=100, validation_data=(xTest, yTest))
# денежный решейп,
...
plt.plot(yhat[0:20, 2], label='predict' + str(3))
plt.legend()
plt.show()

for i in range(count_of_predict):
    plt.plot(yhat[0:100, i], label='predict' + str(i + 1))
    plt.plot(yTest[0:100, i], label='true')
    plt.legend()
    plt.show()
